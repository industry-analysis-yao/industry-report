...
def generate_highlights(items, api_key=None, excluded_urls=None):
    """Build Top-3 highlights from the already-scored items.

    Sorts all scored items by score descending so the absolute top-3 are
    selected regardless of the order items were passed in.  When scores are
    equal the item with the most recent publication date is preferred
    (tie-breaker).  The impact_analysis field generated by Agent B is reused
    ...
    *excluded_urls* is an optional set of URL strings that were already
    ...
    If excluding those URLs would leave fewer than 3 candidates the exclusion
    is relaxed so the highlight block is always fully populated.
    """
...
def main():
    ...
    # Selection happens here — after the full 45-item deduplication above —
    # so it always draws from the freshest batch of the day.
    highlights = generate_highlights(data, excluded_urls=recent_top3_urls) if data else existing_highlights
    if not highlights:
